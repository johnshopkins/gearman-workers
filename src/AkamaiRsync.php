...
class AkamaiRsync
{
  ...
  public function upload(\GearmanJob $job)
  {
    ...
    foreach ($workload->filenames as $filename) {
      ...
      if ($return) {
        $this->logger->addCritical("Failed to rsync file to Akamai. File: {$workload->source}/{$filename}. Rsync returned error `{$return}` in " . __FILE__ . " on line " . __LINE__, array("output" => $output));
      } else {
